...
class Route
{
    private const HTTP403MSG = '403 Forbedden';
    private const HTTP404MSG = '404 Not Found';
    private const HTTP500MSG = '500 Internal Server Error';
    ...
    public function registerRouting()
    {
         ...
         * Top menu
         *************************************************************/
        $this->app->get('/', function (Request $req, Response $res, array $args) {
            return $res->withStatus(303)->withHeader('Location', $this->router->pathFor('top'));
        })->setName('root');

        $this->app->get('/index', function (Request $req, Response $res, array $args) {
         ...
         *************************************************************/
        $this->app->post('/login', function (Request $req, Response $res, array $args) {
        ...
        $this->app->get('/logout', function (Request $req, Response $res, array $args) {
            unset($_SESSION['user']);
            $this->flash->addMessage('message', 'You\'ve successfully logged out.');
            return $res->withStatus(303)->withHeader('Location', $this->router->pathFor('top'));
        })->setName('logout');

        $this->app->post('/register', function (Request $req, Response $res, array $args) {
            $params = $req->getParsedBody();
            $username = filter_var($params['username']);
            $password = filter_var($params['password']);
            try {
         ...
         * Ranking
         *************************************************************/
        $this->app->get('/ranking', function (Request $req, Response $res, array $args) {
            if (Auth::isAuthed($_SESSION)) {
                $tmp = new Ranking();
         ...
         * Stages
         *************************************************************/
        $this->app->group('/stage/{key:[a-f0-9]{40}}', function () {
            $this->get('', function (Request $req, Response $res, array $args) {
                try {
                    ...
                    return $res->withStatus(404)->write(self::HTTP404MSG);
                }
            })->setName('stage');
        ...
        $this->app->group('/admin', function () {
            $this->get('', function (Request $req, Response $res, array $args) {
                if (Auth::isAuthedAdmin($_SESSION)) {
                    ...
                    return $res->withStatus(403)->write(self::HTTP403MSG);
                }
            })->setName('admin');
            ...
            $this->group('/stage', function () {
                $this->get('', function (Request $req, Response $res, array $args) {
                    try {
                        ...
                        return $res->withStatus(500)->write(self::HTTP500MSG);
                    }
                })->setName('admin-stage');
